"""
Inject a ``server`` sub-command into the nanobot Typer CLI.

After installing nanobot-server the user can run:

    nanobot server                     # start server + gateway on default ports
    nanobot server --port 9090         # custom server port
    nanobot server --no-gateway        # server only (external/already-running gateway)
    nanobot server --workspace ~/work  # override workspace

The original ``nanobot`` entry point is shadowed by this module so that the
server sub-command appears alongside the built-in ones (gateway, agent, onboard).
...
def _pid_file() -> Path:
    return Path.home() / ".nanobot" / "server.pid"


def _port_file() -> Path:
    return Path.home() / ".nanobot" / "server.port"


def _log_file() -> Path:
    return Path.home() / ".nanobot" / "server.log"
...
# ── Override `status` to include server process info ──────────────────────────
...
@app.command("status")
def status() -> None:
    """Show nanobot status (including server)."""
    ...
    if running:
        ...
        _con.print(
            f"Server: [green]\u2713 running[/green] "
            f"(PID {pid} \u2022 http://localhost:{port_str})"
        )
        _con.print(f"Log   : {_log_file()}")
    else:
        _con.print("Server: [dim]not running[/dim]")
...
# ── `server` sub-app (supports `nanobot server` and `nanobot server logs`) ───────

server_app = typer.Typer(
    name="server",
    help="Manage the nanobot server.",
...
def server(
    ctx: typer.Context,
    port: int = typer.Option(4096, "--port", "-p", help="Server HTTP port (default: 4096)"),
    gateway_port: Optional[int] = typer.Option(
        None, "--gateway-port", "-g",
        help="nanobot gateway port (default: read from ~/.nanobot/config.json)",
    ),
    host: str = typer.Option("0.0.0.0", "--host", help="Bind address for server"),
    ...
    no_gateway: bool = typer.Option(
        False, "--no-gateway",
        help="Start server only; do NOT start the nanobot gateway/agent (useful when "
             "the gateway is already running externally)",
    ),
    daemon: bool = typer.Option(
        False, "--daemon", "-d",
        help="Run in the background and return immediately. "
             "PID written to ~/.nanobot/server.pid, logs to ~/.nanobot/server.log.",
    ),
):
    """Start the nanobot server (and optionally the gateway) in a single process."""
    ...
    # Apply server patches before importing nanobot internals
    from server.__main__ import _apply_patches, main as _run_all
    _apply_patches()

    if no_gateway:
        # Minimal mode: server HTTP only, no agent/channels.
        ...
        from server.__main__ import _apply_patches, _make_provider
        _apply_patches()
        ...
        from server.api.channel_ext import ExtendedChannelManager
        from server.api.gateway import ServiceContainer, start_api_server
        ...
        typer.echo(f"Starting nanobot server (API only, no gateway) on http://{host}:{port}")
        asyncio.run(start_api_server(container, host=host, port=port))
    else:
        # Full mode: server + gateway + agent + channels (same as python -m server)
        if config_path:
...
@app.command("stop")
def stop() -> None:
    """Stop the background nanobot server process."""
    running, pid = _is_server_running()
    if not running:
        typer.echo("nanobot server is not running.")
        raise typer.Exit(0)
    ...
    typer.echo(f"\u2713 nanobot server stopped (PID {pid})")
...
def _start_daemon(
    ...
    """Spawn a detached nanobot-server process and record its PID."""
    import shutil
    import subprocess

    # Check for an already-running instance
    running, old_pid = _is_server_running()
    if running:
        old_port = _port_file().read_text().strip() if _port_file().exists() else "?"
        typer.echo(
            f"nanobot server is already running (PID {old_pid}, "
    ...
    cmd: list[str] = [nanobot_exe, "server", "--port", str(port), "--host", host]
    ...
    typer.echo(f"\u2713 nanobot server started in background (PID {proc.pid})")
...
def run_nanobot() -> None:
    """Entry point for the ``nanobot`` command (with server subcommand added)."""
    app()


def run_server() -> None:
    """Entry point for the standalone ``nanobot-server`` command."""
...
def _make_standalone_parser():
    ...
    p = argparse.ArgumentParser(
        prog="nanobot-server",
        description="nanobot server — start server + gateway in one process",
    )
    p.add_argument("--port", type=int, default=4096, help="Server port (default: 4096)")
    ...
    p.add_argument("--no-gateway", action="store_true", dest="no_gateway",
                   help="Start server only (skip gateway/agent)")
    return p
