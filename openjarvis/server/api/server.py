...
from server.api.middleware import setup_cors
...
def create_app(container: ServiceContainer | None = None) -> FastAPI:
    ...
    # Resolution order:
    #   1. Editable install: <repo>/server/web/dist/
    #   2. Installed wheel:  importlib.resources traversal (works with zipimport too)
    _here = Path(__file__).parent  # server/api/
    web_dist = _here.parent / "web" / "dist"  # editable: server/web/dist
    if not web_dist.exists():
        try:
            import importlib.resources as _ir
            _traversable = _ir.files("server").joinpath("web/dist")
            ...
            if _candidate.exists():
                ...
                _tmp = Path(tempfile.mkdtemp(prefix="nanobot_server_dist_"))
    ...
    if web_dist.exists():
        ...
        async def spa_fallback(full_path: str):  # noqa: ARG001
            ...
            return {"message": "Frontend not built. Run 'bun run build' in server/web/"}
