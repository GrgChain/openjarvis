...
Starts nanobot gateway + FastAPI server in a single asyncio process.
...
def _apply_patches() -> None:
    """Monkey-patch nanobot behaviours that the server overrides.

    Rules:
    - Empty allow_from → allow all (server manages access via the UI;
    ...
    # Patch 2: _validate_allow_from — no-op (server users configure via UI)
...
def _make_provider(config):
    ...
    if not model.startswith("bedrock/") and not (p and p.api_key) and not (spec and spec.is_oauth):
        print(
            ...
            "Start the server and set one in Settings → Providers.",
            file=sys.stderr,
        )
...
def main_cli() -> None:
    """Entry point for the ``nanobot-server`` console script."""
    import argparse
    ...
    parser.add_argument("--port", type=int, default=4096, help="Server port (default: 4096)")
    ...
    parser.add_argument("--daemon", "-d", action="store_true", default=False,
                        help="Run in the background (PID → ~/.nanobot/server.pid)")
    args = parser.parse_args()

    if args.daemon:
        from server.cli import _start_daemon
        _start_daemon(
            port=args.port,
